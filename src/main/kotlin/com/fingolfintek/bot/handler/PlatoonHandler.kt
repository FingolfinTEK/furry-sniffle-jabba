...
open class PlatoonHandler(
    ...
    Try.ofSupplier { messageRegex.matchEntire(message.content)!! }
        .andThen(Consumer {
          ...
          characterRepository.searchByName(it.groupValues[2])
              ...
              .onEmpty {
                message.channel
                    .sendMessage("No members found that have ${characterRepository.searchByName(it.groupValues[1]).get()} at $rarity*")
                    .queue()
              }
        })
  ...
  private fun toPriorityListOfMembersHavingToonAt(
      channelId: String, toon: Character, rarity: Int, limit: Int): String {
